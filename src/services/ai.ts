...
import type { Company, SitePrompts } from '../lib/types'
...
export async function generateSitePrompts(company: Company): Promise<SitePrompts> {
  const language = getCountry(company.country).language
  const { data, error } = await supabase.functions.invoke<{ prompts: SitePrompts }>(
