...
import type { Company, Lead, LeadStatus } from '../lib/types'
...
export async function addLeadFromCompany(company: Company): Promise<Lead> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      status: 'novo' satisfies LeadStatus,
      notes: '',
